...
export function spawnPlayer(world: SimWorld): number {
  ...
  // 90% de la vitesse max en ~120 ms (≈ 7 images à 60 Hz), arrêt en ~80 ms.
  // Ces constantes ont fait un aller-retour : réglées d'abord deux fois trop vives
  // (~67/~50 ms) par contradiction interne du plan, conservées ainsi sur la foi de
  // maquettes, puis ramenées ici après le premier essai réel — « trop sec, pas assez
  // de poids ». Les tests encadrent la cible des deux côtés, donc toute dérive de ce
  // ressenti fera échouer la suite au lieu de passer inaperçue.
  // NE PAS recalculer ces valeurs à la main : régler jusqu'à satisfaire les tests.
  Movement.accel[eid] = PLAYER_SPEED / 0.093
  Movement.friction[eid] = PLAYER_SPEED / 0.056
...
```ts
    ...
    // On décrémente AVANT d'appliquer la vitesse : sinon la dernière image
    // pousse le joueur à 720 px/s tout en retirant `Dashing`, donc sans
    // l'invulnérabilité ni le pouvoir de tuer qui vont avec. La ruée s'arrête
    // franchement plutôt que de laisser une image orpheline à pleine vitesse.
...
avec les imports `hasComponent`, `removeComponent` et `Dashing`.

La ruée doit aussi tuer ce qu'elle traverse. Plutôt que de greffer un cas
particulier dans `hazardSystem` — qui traite des zones, or la ruée n'en est pas
une — on lui consacre un système dédié : sa condition d'activation (le joueur
porte `Dashing`) n'a rien à voir avec la logique des zones, et les mélanger
rendrait les deux plus difficiles à lire et à tester.
